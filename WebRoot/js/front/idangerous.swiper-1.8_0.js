...
Swiper = function(selector, params, callback) {
	...
	function onTouchEnd(event) {
		/*var idN = document.getElementById("secondD");
		var stringValue = idN.style.transform;
		//var patrn = new  RegExp('^translate3d\(0px,\s(-?\d+)px,\s0px\)$')
		var patrn = /^translate3d\(0px,\s-(\d+)px,\s0px\)$/;
		var r = patrn.exec(stringValue);
		var ir = r[1]
		alert(ir);*/
		...
		if (params.preventLinks) { 
		    var flag = true;
				...
				if(!idCode==""){
					//alert(s)
					/*var idN = document.getElementById(idCode);
			        var stringValue = idN.style.transform;
			        var patrn = /^translate3d\(0px,\s-?(\d+)px,\s0px\)$/;
			        var r = patrn.exec(stringValue);
			        //console.log(r)
					...
					var indexV = ir/38;
					//alert(indexV)
					//alert($("#"+idCode+" .swiper-slide").eq(indexV+1).html())
					$("#"+idCode+" .swiper-slide").eq(indexV+1).addClass("clickThisTime").siblings().removeClass("clickThisTime");
				 	//$(".firstDiv .swiper-slide").eq(indexV).addClass("clickThisTime").siblings().removeClass("clickThisTime");
				 	//$(".swiper-container .clickThisTime").css({"color": "#333"})
					provinceId = $("#province .swiper-slide").eq(indexV+1).children("span").attr("name");*/
				   flag = false;
				}else{
					return false;
				}
			...
			if(flag){
				// $(".swiper-slide").eq(1).addClass("clickThisTime").siblings().removeClass("clickThisTime");
				$(".firstDiv .swiper-slide").eq(0).addClass("clickThisTime").siblings().removeClass("clickThisTime");
			}
		}
		//onSlideClick
